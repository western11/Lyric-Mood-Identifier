...
# Model Tuning
Sadly i'm not satisfy about the result. the highest Accuracy only 62.6%. i will try to tune some models in hope we can get better result

```{r}
# build 5 folds cross validation for tuning evaluationn
## the column names like break,for,next,if are considered as special character thus raises an eror when tuning.
...
mod.dt.2x <- decision_tree(mode = "classification",
                           cost_complexity = 0.01,tree_depth = 23,min_n = 20) %>%
...
As we can see from the table above, `Naive Bayes` model has the highest Accuracy. Our tuned decision tree doesn't even give any different. It’ll always possible to have higher accuracy (or other metrics) if we try another classification model. We’ll do that in the future. So in conclusion, **we'll use Naive Bayes model to predict song's mood based on its lyric.**
...
Our best model is actually Naive Bayes. we'll also build function to clean up the text and matching it to Naive Bayes requirement. the only different is in the last step we apply benoulli converter and return it as transposed matrix.
...
```
Naive Bayes Model are also predict the lyric as negative mood music. if you hear the real song, its actually a spirit, energic, and positive mood music but i never know what is the lyrics actually say.
